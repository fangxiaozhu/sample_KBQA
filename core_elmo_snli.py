...
model = SNLIModel()
model.load_state_dict(torch.load(config.snli_model_path))
...
class answerCandidate:
    ...
    def calcScore(self, qtList, countCharDict, debug=False, includingObj = [], use_elmo=False):
        ...
        if use_elmo and len(pre) != 0:
            ...
            scorePre = model(pre_embedding, q_embedding).item() # 2 * 1024   snli模型计算相似度分数
...
if __name__ == '__main__':
    ...
    pathOutput = config.result_snli_path
